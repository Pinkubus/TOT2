...
type ImgItem = {
  id: string;
  name: string;
  dataUrl: string; // persisted image (base64 data URI)
...
function updateElo(winner: ImgItem, loser: ImgItem) {
...
// --- Main Component ---
export default function ImageThisOrThat() {
  const [items, setItems] = useLocalStorage<ImgItem[]>("tot-items", []);
  const [seenPairs, setSeenPairs] = useLocalStorage<Record<string, number>>("tot-pairs", {}); // pairKey -> count
  const [history, setHistory] = useState<MatchRecord[]>([]);
  const fileInputRef = useRef<HTMLInputElement>(null);

  // --- Upload handling ---
  async function filesToItems(files: FileList | File[]) {
  ...
  function getNextPair(): [ImgItem, ImgItem] | null {
  ...
  const [currentPair, setCurrentPair] = useState<[ImgItem, ImgItem] | null>(null);
  useEffect(() => {
  ...
  function resetAll() {
    if (!confirm("Clear all images and progress?")) return;
    setItems([]);
    setSeenPairs({});
    setHistory([]);
    setCurrentPair(null);
  ...
  function clearRatings() {
    if (!confirm("Reset all ratings and match history (keep images)?")) return;
...
function Leaderboard({ items }: { items: ImgItem[] }) {
...
function Gallery({ items, onRemove }: { items: ImgItem[]; onRemove: (id: string) => void }) {
  return (
